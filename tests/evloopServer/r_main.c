...
#define SERVER_NAME_NIM "NIM-LANG/"
...
H2O_NORETURN static void *run_loop(void *_thread_index)
{
    ...
    fprintf(stderr, "Shutting down thread\n");
...
int main(int argc, char **argv)
{
    ...
    conf.globalconf.server_name = h2o_iovec_init(H2O_STRLIT("h2o/server" /*SERVER_NAME_NIM H2O_VERSION*/));
    ...
    fprintf(stderr, "Num_threads to start: #%zu\n",conf.num_threads);
    ...
    if (phostconf)
    {
    configure_router(h2o_hostconf_t **hosts_ptr, h2o_globalconf_t *global_ptr)
    register_handler(phostconf, "/api/{dArling:[a-zA-Z]+}/{id}", METHOD_GET, chunked_test);
    fprintf(stderr, "Compile routes\n");
    ...
    fprintf(stderr, "Stoped all threads\n");
